...
> Статус: этапы 1–2 реализованы (реестр `src/utils/activatedEffects.ts`, секция
> `src/components/ActiveFormsSection.tsx`, ресурсы подклассов, миграция гибридных форм,
> linked-эффекты, expiresAt, снятие по Incapacitated/отдыхам). Этап 3 (интеграция
> с бросками) — не реализован, опционален.
> Отличия от плана по фактическим текстам фич: Bladesong требует «без среднего/тяжёлого
> доспеха» (лёгкий разрешён); Invincible Conqueror — 1 минута; аватары L20 — три
> отдельных ключа (conquest/crown/vengeance); добавлена мутация Slimy Form.

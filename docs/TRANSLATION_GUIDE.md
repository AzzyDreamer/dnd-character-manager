...
| `@class` | `{@class Wizard\|XPHB}` | Ссылка на класс |
| `@subclass` | `{@subclass Bladesinger\|Wizard\|TCE}` | Ссылка на подкласс |
...
| `@itemProperty` | `{@itemProperty L\|XPHB\|Light}` | Ссылка на свойство предмета |
...
**Исключение — `@subclass`:** у него сегменты `Название|Класс|Источник|Склонение`, поэтому
отображаемый текст для склонения — это **четвёртый** параметр, а не третий:

```
{@subclass Bladesinger|Wizard|TCE|Песнь клинка}
```

У `@class` склонение, как обычно, в третьем параметре: `{@class Wizard|XPHB|волшебника}`.
...
Внимание: у `@filter` **первый параметр — это уже видимый текст**, а не «название|источник|склонение» как у ссылок выше. Категория и параметры фильтра не отображаются, но используются для перехода: клик по `@filter` открывает нужную категорию глоссария с применённым фильтром (если категория поддерживается приложением — feats/spells/items/optionalfeatures/species/backgrounds/conditions/actions/rules). Для неподдерживаемых категорий текст просто показывается без клика.
...
### Теги-имена без ссылки

Эти теги показывают **имя сущности**, но кликнуть по ним нельзя — данных в приложении
нет. Отображается приглушённым курсивом. Переводится (через третий параметр для
склонения), но ссылки не будет:

| Тег | Пример |
|-----|--------|
| `@trap` | `{@trap Danse Macabre\|VRGR\|Пляской смерти}` |

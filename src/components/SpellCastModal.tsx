...
function getSpellMeta(spellData: SpellData) {
...
export const SpellCastModal: React.FC<SpellCastModalProps> = ({
  ...
  const meta = getSpellMeta(spellData);
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-border-default flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-text-muted mt-0.5">
              ...
              {spellData.school && ` • ${SCHOOL_NAMES[spellData.school] || spellData.school}`}
              {ritual && ' • Ритуал'}
            </div>
          </div>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
            {meta.castingTime && <div><span className="text-text-muted">Время: </span><span className="text-text-primary">{meta.castingTime}</span></div>}
            {meta.range && <div><span className="text-text-muted">Дальность: </span><span className="text-text-primary">{meta.range}</span></div>}
            {meta.components && <div><span className="text-text-muted">Компоненты: </span><span className="text-text-primary">{meta.components}</span></div>}
            {meta.duration && <div><span className="text-text-muted">Длительность: </span><span className="text-text-primary">{meta.duration}</span></div>}
          </div>

          {/* Slot selection (leveled spells only) */}
          {!isCantrip && (
            <div>
              <div className="text-xs text-text-muted uppercase tracking-wider mb-2">Ячейка заклинания</div>
          ...
          {hasAttack && (
            <div className="flex items-center gap-3 p-3 rounded-lg bg-bg-secondary/50 border border-border-default">
              ...
              <span className="text-sm text-text-secondary">Бросок атаки</span>
          ...
          {hasSave && (
            <div className="flex items-center gap-3 p-3 rounded-lg bg-bg-secondary/50 border border-border-default">
              ...
              <span className="text-sm text-text-secondary">Спасбросок</span>
              <span className="ml-auto text-sm">
                <span className="text-gold font-bold text-lg mr-1.5">СЛ {spellDC}</span>
          ...
          {damageInfo && (
            <div className="flex items-center gap-3 p-3 rounded-lg bg-bg-secondary/50 border border-border-default">
              ...
              <span className="text-sm text-text-secondary">
                {damageInfo.isHealing ? 'Лечение' : 'Урон'}
              </span>
        ...
        <div className="px-5 py-3 border-t border-border-default flex items-center justify-between">
          <div className="text-xs text-text-muted">
            {isCantrip
              ? 'Заговор — ячейка не требуется'
              : isRitualCast
              ? 'Ритуал — ячейка не тратится'
              : `Ячейка ${ROMAN[selectedSlotLevel - 1]} уровня`}
          </div>

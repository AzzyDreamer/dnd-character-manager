...
export interface Character {
  ...
  // Размер сетки рюкзака (по умолчанию 10×8). Игрок может расширять вручную:
  // сперва по столбцам (пока влезает по ширине), затем по строкам.
  inventoryRows?: number;
  inventoryCols?: number;
  // Отключить учёт переносимого веса/грузоподъёмности (не показывать перегруз).
  disableEncumbrance?: boolean;
  // Отключить лимит настройки магических предметов (по умолчанию 3).
  disableAttunementLimit?: boolean;
